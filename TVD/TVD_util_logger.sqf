...
if (count _this >= 2) then {
	switch (_this select 1) do {
		...
		case 2 : { _outcome =  format ["Сторона понесла тяжелые потери: %1", TVD_HeavyLosses]};
...
_si1 = [east, west, resistance, civilian, sideLogic] find (TVD_sides select 1);
		...
		diag_log	parseText format ["Soldiers Dead: %1 - %2;   Soldiers Alive: %3 - %4", (wmt_playerCountInit select _si0) - (wmt_PlayerCountNow select _si0), (wmt_playerCountInit select _si1) - (wmt_PlayerCountNow select _si1), wmt_PlayerCountNow select _si0, wmt_PlayerCountNow select _si1];
